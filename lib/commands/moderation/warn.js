...
exports.run = async (client, interaction) => {
    ...
    if (!action || !["add", "list", "remove"].includes(action))
        return interaction.sendSilent("Stop breaking shit >:(");
    if (action === "add") {
        const memberId = interaction.args[0].options.find((arg) => arg.name === "member")
            ?.value;
        ...
        if (!id) {
            return interaction.send(new Embeds_1.ErrorEmbed("Please enter an id of a warning to remove"));
        }
    }
};
exports.config = {
